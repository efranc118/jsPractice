function findRepeatedNum(A) {
  ...
  //         actualSum += A[i];
  //     }
  //     var result = actualSum - totalSum;
  //     return result;

  let ranges = [];
  let sqrt = Math.floor(Math.sqrt(A.length - 1));
  if(sqrt * sqrt < A.length - 1) sqrt++;
  for (i = 0; i < A.length; i++) {
    let bucket = Math.ceil((A[i] - 1) / sqrt);
    if (ranges[bucket] === undefined) {
      ranges[bucket] = 1;
    } else {
      ranges[bucket]++;
    }
  ...
  let min = ((largestBucket - 1) * sqrt);
  let max = largestBucket * sqrt;
  let miniMap = {};
  for (i = 0; i < A.length; i++) {
    if (A[i] > min && A[i] <= max) {
      if (miniMap[A[i]] !== undefined) return A[i];
      miniMap[A[i]] = 1;
    }
  }
  return -1;
...
console.log(findRepeatedNum(testArray));